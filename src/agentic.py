...
class AgenticMemoryProcessor:
    ...
    async def plan_merge(self, memories: List[MemoryEntry]) -> Optional[Dict[str, Any]]:
        ...
        prompt = ChatPromptTemplate.from_messages([
            (
                "system",
                "You are a memory pruning assistant. You identify redundant information "
                "and merge it into a single, comprehensive record.",
            ),
            (
                "human",
                (
                    "Review these memories and determine if any of them are redundant "
                    "(describing the same fact/preference/event with high overlap).\n\n"
                    "Memories:\n{memories_text}\n\n"
    ...
    async def orchestrate_lifecycle(
        self, ltm: Any, entry: MemoryEntry, content: str
    ) -> None:
